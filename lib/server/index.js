...
_addPlaceholders = function(content, section) {
  // TODO: Reference 'general' from Section class
  let prefix = section.name !== 'general' ? `${section.name}::` : ''

  if (content.length == 0) {
    let placeholders = Utils.reduce(section.fields, (memo, params, token) => {
    ...
    Utils.each(content, (record) => {
      Utils.each(record, (value, key) => {
        let name = section.fields[key].name

        if (Utils.isEmpty(value) && name) {
          record[key] = `{{${prefix}${placeholder}}}`
        }
      })
    })
  }
...
_injectLiveReload = function(ctx) {
  const URL = url.parse(ctx.request.origin)
  const script = `<script src="/dashboard/javascripts/livereload.js?snipver=1&port=${URL.port}&host=${URL.hostname}"></script>`
