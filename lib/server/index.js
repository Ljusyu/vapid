...
class Server {
  constructor (site, options = {}) {
    ...
    this.siteFavicon = path.join(this.siteAssets, 'favicon.ico')

    this.dashboardAssets = path.resolve(__dirname, '../../assets')
    this.dashboardViews = path.resolve(__dirname, '../../views')
    this.dashboardFavicon = path.join(this.dashboardAssets, 'favicon.ico')

    // TODO: How to pass in other configs?
    this.db = new Database({
      dialect: 'sqlite',
      storage: path.join(this.siteRoot, 'db', `${app.env}.sqlite`)
    })

    app.context.models = this.db.models

    app
      // TODO: Allow local site to override favicon
      .use(favicon(this.dashboardFavicon))
    ...
    app.use(async ctx => {
      ...
      for (let [token, groupAttrs] of Object.entries(tree)) {
        let group = await ctx.models.Group.findOne({ where: { name: groupAttrs.name}, include: 'records' })
  ...
  start () {
    this.db.connect(app.env == 'development')
