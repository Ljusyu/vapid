...
class Server {
  ...
  start() {
    ...
    vapid.models.Record.addHooks(['afterSave', 'afterDestroy'], () => {
      cache.clearPrefix(cache_path_prefix)
      this.watcher && this.watcher.refresh()
    })
  }
...
_websocketPort = function(ctx) {
  let forwarded = ctx.header['x-forwarded-proto']
  let protocol = forwarded ? forwarded.split(',')[0] : undefined
