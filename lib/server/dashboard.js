...
router.use(views(router.paths.templates, {
  extension: 'ejs',
  map: {
    html: 'ejs'
  }
...
router.use(async (ctx, next) => {
  if (ctx.isAuthenticated()) {
    // For the nav menu
    ctx.state.allSections = await vapid.models.Section.findAll()
...
router.post('records#create', '/sections/:id/records', findSection, async ctx => {
  ...
  try {
    ...
    record = await vapid.models.Record.build({ content: _content(ctx.request), section_id: section.id })
...
router.post('records#update', '/records/:id', findRecord, async ctx => {
  try {
    ...
    ctx.flash('success', `Updated ${record.section.labelSingular}`)
...
function _saveFile(file) {
  ...
  const savePath = path.join(vapid.config.uploads, fileName)

  // Just in case
  Utils.mkdirp(vapid.config.uploads)
