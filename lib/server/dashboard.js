...
router.use((ctx, next) => {
  ...
  ctx.response.render = ctx.render = (relPath, title, locals = {}) => {
    ...
    return render(`layouts/${layout}`, locals)
  }
...
router.get('auth#install', '/install', async ctx => {
  ...
  return ctx.render('auth/install', 'Install', {
    email: ""
  })
})
...
router.get('auth#sign_in', '/sign_in', async ctx => {
  ...
  return ctx.render('auth/sign_in', 'Sign In')
})
...
router.get('build', '/build', async (ctx, next) => {
...
router.get('records#index', '/groups/:id/records', rescueBuildRedirect, findGroup, ctx => {
  return ctx.render('records/index', ctx.state.group.label)
})

router.get('records#new', '/groups/:id/records/new', rescueBuildRedirect, findGroup, ctx => {
  let title = ctx.state.group.repeating ? `New ${ctx.state.group.labelSingular}` : ctx.state.group.label
  return ctx.render('records/new', title, {
...
router.get('records#edit', '/records/:id/edit', rescueBuildRedirect, findRecord, ctx => {
  let title = `Edit ${ctx.state.record.group.labelSingular}`
  return ctx.render('records/edit', title, {
...
router.get('records#delete', '/records/:id/delete', findRecord, ctx => {
  ...
  return ctx.render('records/delete', `Delete ${title}`)
})
