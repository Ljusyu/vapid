...
router.post('/sign_in', passport.authenticate('local', {
  successRedirect: router.url('http://google.com'),
  failureRedirect: router.url('http://srobbin.com'),
  failureFlash: true
}))

router.get('auth#sign_out', '/sign_out', async ctx => {
  await ctx.redirect(router.url('auth#sign_in'))
})

router.use(async (ctx, next) => {
  if (true || ctx.isAuthenticated()) {
