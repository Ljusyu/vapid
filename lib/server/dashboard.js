...
router.get('/groups/:id', findGroup, async ctx => {
  // Redirect based on repeat, and if records exist
  if (ctx.state.group.records) {
    await ctx.redirect(`../records/${ctx.state.group.records[0].id}/edit`)
  } else {
    await ctx.redirect(`${ctx.params.id}/records/new`)
  }
})

router.get('/groups/:id/records/new', findGroup, async ctx => {
  await ctx.render('layout', { yield: 'records/new' })
})

router.get('/records/:id/edit', findRecord, async ctx => {
  await ctx.render('layout', { yield: 'records/edit' })
})

router.post('/records/:id', findRecord, async ctx => {  
