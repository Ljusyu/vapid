...
passport.deserializeUser(async (id, done) => {
  try {
    vapid.currentUser = await vapid.models.User.findById(id);
    done(null, vapid.currentUser);
  } catch (err) {
    done(err);
  }
});
...
passport.use(new LocalStrategy({ usernameField: 'email' }, async (email, password, done) => {
  const user = await vapid.models.User.findOne({ where: { email } });
...
router.post('/install', async (ctx) => {
  ...
  try {
    const user = await vapid.models.User.create({
...
async function findSection(ctx, next) {
  const section = await vapid.models.Section.findById(ctx.params.id, { include: 'records' });
...
async function findRecord(ctx, next) {
  const record = await vapid.models.Record.findById(ctx.params.id, { include: 'section' });
