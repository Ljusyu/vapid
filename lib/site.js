...
class Site {
  ...
  /**
   * load the site's database
   * todo: make this async
   */
  initialize(cb) {
    waterline.initialize(dbConfig, (err, orm) => {
      this.seedDB(orm.collections)
      cb(err, orm.collections)
    })
  }

  /**
   * unload the site's database
   * todo: make this async
   */
  teardown() {
    waterline.teardown()
   ...
   */
    ...
    */
    async seedDB(models) {
      let defaultGroup = await models.group.firstOrCreate({ name: "default" })
    }
}
