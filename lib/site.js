...
class Site {
  ...
  async build() {
    ...
    for (let [name, attrs] of Object.entries(tree)) {
      if (Object.keys(attrs.fields).length === 0) continue
...
function _createTemplateTree(templates) {
  ...
  for (let tpl of templates) {
    ...
    for (let [groupToken, groupAttrs] of Object.entries(parsed)) {
      ...
      Object.assign(tree[groupName].options, groupAttrs.attributes)

      for (let [fieldToken, fieldAttrs] of Object.entries(groupAttrs.fields)) {
        let fieldName = fieldAttrs.name
        tree[groupName].fields[fieldName] = Object.assign({ type: 'text' }, tree[groupName].fields[fieldName] || {}, fieldAttrs.attributes)
      }
    }
  }
